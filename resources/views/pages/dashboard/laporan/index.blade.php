...
@section('page_description', 'Analisis data performa penanganan pengaduan dan ekspor rekapitulasi resmi.')

@section('content')
<div class="space-y-6">
    
    <!-- Filter Date Range & Parameters (Sheaf UI Card) -->
    <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-5 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-zinc-800/80">
            <h3 class="font-semibold text-xs text-zinc-200 flex items-center gap-2">
                <i data-lucide="filter" class="w-3.5 h-3.5 text-orange-400"></i>
        ...
        <form action="{{ route('dashboard.laporan.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div>
                ...
                <input type="date" name="start_date" value="{{ $startDate }}" 
                       class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-100 focus:outline-none focus:border-orange-500">
            </div>
            ...
            <div>
                ...
                <select name="status" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-100 focus:outline-none focus:border-orange-500">
            ...
            <div class="flex items-end">
                <button type="submit" 
                        class="w-full bg-zinc-900 hover:bg-zinc-800 text-zinc-100 border border-zinc-700 font-medium py-1.5 rounded-lg text-xs transition-all">
                    Terapkan Filter
                </button>
            </div>
        </form>
    ...
    <!-- Summary Stats Metrics (Sheaf 5-Grid) -->
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-3.5 text-center">
        ...
        <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-3.5 text-center">
            <div class="text-[11px] font-mono text-amber-400">Menunggu</div>
            <div class="text-xl font-mono font-semibold text-amber-400 mt-1">{{ $stats['menunggu'] }}</div>
        </div>
        <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-3.5 text-center">
            <div class="text-[11px] font-mono text-orange-400">Diproses</div>
            <div class="text-xl font-mono font-semibold text-orange-400 mt-1">{{ $stats['diproses'] }}</div>
        </div>
        <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-3.5 text-center">
            <div class="text-[11px] font-mono text-emerald-400">Selesai</div>
    ...
    <!-- Report Table (Sheaf UI Data Table) -->
                    ...
                    @forelse($complaints as $c)
                        <tr class="hover:bg-zinc-900/40 transition-colors">
                            <td class="px-4 py-3 font-mono font-semibold text-zinc-300">
                                <span class="bg-zinc-900 px-2 py-0.5 rounded border border-zinc-800">
                                    {{ $c->ticket_code }}
                                </span>
                            </td>
